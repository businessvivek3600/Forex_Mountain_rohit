...
Future<void> logOut(
  ...
  if (showD) {
    showDialog(
        context: ForexMountains.navigatorKey.currentContext!,
  ...
  await Future.delayed(Duration(milliseconds: showD ? 2000 : 0), () async {
    ...
    Navigator.pop(ForexMountains.navigatorKey.currentContext!);
  });
...
/*      var runKey =  AppConstants.testMode ? AppConstants.testCanRun : AppConstants.canRun;
      await firestore  .collection('forex_mountain')  .doc('runApp')  .snapshots()  .listen((event) {if (event.data() != null) {  if ((event.data())!.entries.isNotEmpty) {    canRunApp = (event.data())!        .entries        .firstWhere((element) => element.key == runKey)        .value;  }}
      });*/

/*    await firestore.collection('forex_mountain').doc('version').snapshots().listen((event) {
...
String createDeepLink({String? sponsor, String? placement}) {
  ...
  var msg =
      'Hi 👋 ,\nStarting your Forex Mountains membership can be a fun and exciting endeavour.\neagle.forexmountains.com is a tight-knit community of car enthusiasts who come together to share their love for anything on wheels.\nYou get to meet gearheads, wizards, experts, professionals, and fans of the automotive industry.\n\n Click the below link to join \n${(url.toString())}';
  return msg;
}
