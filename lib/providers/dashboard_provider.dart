...
// company trade ideas
  ...
  Future<void> uploadSteps(Map<String, dynamic> data) async {

    try {
      if (isOnline) {
        showLoading(useRootNavigator: false);
        ApiResponse apiResponse = await dashBoardRepo.uploadDailySteps(data);
        Get.back();
        print("__________________Stepper APi");
        print(data);
        if (apiResponse.response != null &&
            apiResponse.response!.statusCode == 200) {
          var data = apiResponse.response!.data;
          Toasts.showSuccessNormalToast(data["message"]);
        }
      } else {
        Toasts.showWarningNormalToast('You are offline');
      }
